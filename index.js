...
async function completeRegistration(ctx, lead) {
  ...
  await ctx.reply(CHANNEL_LINK);
  await ctx.reply(
    "Endi muhim xabarlarimni o'tkazib yubormasligingiz uchun kanalni\nPIN 📌 qiling va ovozni yoqib qo'ying!"
  );
...
bot.start(async (ctx) => {
  ...
  await ctx.reply(
    "Kanalga qo'shilishingizga bir qadam qoldi:\n\n" +
